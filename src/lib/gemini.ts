...
const KEYWORD_MAP: Record<NewsCategory, string[]> = {
  RISK: ["제재", "과태료", "개인정보", "유출", "소송", "부도", "영업정지", "벌금", "사기", "횡령", "파산", "채무", "압수수색", "수사", "기소", "검찰", "경찰", "적자", "손실", "해킹", "장애", "먹통", "피해", "불법", "위반", "징계", "해고", "폐업", "회생"],
...
**핵심 원칙: 금융/제휴 키워드가 없더라도 해당 업체의 신뢰도, 재무건전성, 평판, 사업 지속성에 영향을 줄 수 있는 기사라면 반드시 포함해주세요.**
예를 들어: 대표 관련 수사/구속, 회사 적자/채무, 개인정보 사고, 시스템 장애, 소비자 피해, 노사갈등, 핵심인력 이탈 등은 금융 제휴와 직접 관련이 없어도 계약 업체의 리스크로 분류해야 합니다.

각 기사를 다음 5가지 카테고리 중 하나로 분류해주세요:
- RISK (리스크): 제재, 과태료, 개인정보유출, 소송, 부도, 영업정지, 수사, 압수수색, 채무, 적자, 시스템장애, 소비자피해, 해킹, 파산, 회생 등 업체 신뢰도에 부정적 영향을 미치는 모든 이슈
- REGULATORY (규제): 금융위, 금감원, 규제, 법률개정, 시정명령
- MANAGEMENT (경영변동): 인수합병, 대표교체, 구조조정, 매각, 철수, 핵심인력 이탈, 노사갈등
